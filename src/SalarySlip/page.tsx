...
return Deduction?.reduce((prev,a)=>( prev+Number(a?.amount!)),0)
  ...
  return (
    ...
    {image == null ?   <div className="add-image" onClick={():void=>document.getElementById("hidden-input")?.click()}>
        ...
        <img src={URL.createObjectURL(image)} alt='cc' width={150} height={150} style={{objectFit:'contain'}} />
    ...
    <input value={"EMPOLOYEE NO"}   style={{fontWeight:700,borderBottom:'none'}} placeholder='Write'></input>
      <div >
    ...
    <div className='invoice-div' >
    <input value={"DATE OF SALARY"}   style={{fontWeight:700,borderBottom:'none'}} placeholder='Write'></input>
    ...
    { inputvalue?.map((e)=>{
      ...
      <input placeholder='input Name' className='' name="name" value={e?.name} style={{borderBottom:'none',color:"BLACK",textTransform:'uppercase',fontWeight:700}} onChange={(event)=>handleinputfill(event,e?.id,inputvalue,setInputvalue)}></input>
...
<div className='left'>
  ...
  {Earning?.map((a)=>{
    ...
    <input value={a?.name} name="name" onChange={(e)=>handleinputfill(e,a?.id,Earning,setEarning)}  style={{fontWeight:700,borderBottom:'none',flex:1}} placeholder='Write'></input>
    ...
    <input value={"Total Amount Earn "} name="name" disabled style={{fontWeight:700,borderBottom:'none',flex:1}}></input>
    <span>₹</span><input type="number" style={{borderBottom:!print ? '1.5px solid grey':'none'}}  value={totalEarning} disabled></input>
</div>
    <div className=' salary-details between-div' >
    <input value={"Total Salary"} name="name"  style={{fontWeight:700,borderBottom:'none',flex:1}}></input>
...
<h5 style={{background:"#4f15d0",padding:'1rem',marginBottom:0,color:'white',borderRadius: '12px 12px 0 0  '}}>Deduction</h5>
  ...
  {Deduction?.map((a)=>{
    ...
    <div className=' salary-details between-div' >
    <input value={"Total Amount Dedc "} disabled name="name"  style={{fontWeight:700,borderBottom:'none',flex:1}}></input>
   ...
   <div className='d-flex justify-content-evenly ' style={{padding:'3rem 0',maxWidth:800,margin:'0 auto ',fontSize:18,fontWeight:700}}>
 ...
 <div>Total Deducction <br/> ₹{totalDeduction.toFixed(2)} </div>
   ...
   <div className='center-div' style={{gridGap:30,margin:"3rem 0",fontWeight:600}}>
...
email to careers@TalkCharge.com. </p>
   ...
   <p style={{margin:' 2rem 0 2rem auto',width:'fit-content',color:'grey',fontWeight:500}}>Generated by Techninza </p>
